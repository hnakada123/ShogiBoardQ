...
人間対エンジンの投了後の棋譜操作は `engineHumanResignNavigation` で検証します。人間が先手・後手の両方で、投了直後も手番ラベル・対局者名・持ち時間の背景が黄色を保つことを確認します。その後、指し手列・消費時間列から開始局面・途中の手・投了行を繰り返し選び、盤面・手番・黄色の行が選択と一致することを確認します。
