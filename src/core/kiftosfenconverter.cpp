                                                                ...
                                                                QString* errorMessage)
{
    ...
    for (auto it = lines.cbegin(); it != lines.cend(); ++it) {
        ...
        // 行末の時間 "( m:ss / H+:MM:SS )" を取り出し（半角/全角の区切り両対応）
        QRegularExpressionMatch tm = kifTimeRe().match(rest);
        QString timeText;
        if (tm.hasMatch()) {
            timeText = normalizeTimeMatch_(tm); // "mm:ss/HH:MM:SS"
