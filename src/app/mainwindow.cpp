...
{
    // 既存インスタンスの後片付け
    if (m_usi1 != nullptr) { delete m_usi1; m_usi1 = nullptr; }
    ...
    // この対局モードでエンジンが担当する手番を判定
    // 平手(EvenHumanVsEngine):   エンジン=P2
    // 駒落ち(HandicapEngineVsHuman): エンジン=P1
    const bool engineIsP1 =
        (m_playMode == HandicapEngineVsHuman);

    // 重複接続防止：事前に切ってから張る
    QObject::disconnect(m_usi1, &Usi::bestMoveResignReceived, this, &MainWindow::onEngine1Resigns);
    QObject::disconnect(m_usi1, &Usi::bestMoveResignReceived, this, &MainWindow::onEngine2Resigns);
    if (engineIsP1) {
    ...
    if (m_playMode == EvenHumanVsEngine) {
        ...
        // 念のため（この関数では通常使わないが、元コードに合わせて残すならP2エンジン）
        initializeAndStartPlayer2WithEngine1();
    }

    // 初期 position（ponder 用も確保）
    m_positionStr1 = "position " + m_startPosStr + " moves";
    m_positionStrList.append(m_positionStr1);
    m_positionPonder1 = m_positionStr1;

    // 盤クリックの受付（重複しないよう貼り直し）
    QObject::disconnect(m_shogiView, &ShogiView::clicked,
    ...
    // ★ 初手が人間のときは go を出さず UI を整えて戻る
    if (humanTurnNow) {
        // 人間手番のストップウォッチを UI 安定後にアーム
    ...
    // --- ここから初手がエンジンのとき ---
    QPoint outFrom(-1, -1), outTo(-1, -1);

    // この手の思考上限（ms）をメモ（旗落ち判定用）
    ...
    if (isMoveValid) {
        ...
        // 直前に指した側（= エンジン）の思考時間を UI に反映
        if (m_gameController->currentPlayer() == ShogiGameController::Player1) {
            // validateAndMove 後の現在手番=先手 → 直前は後手（= エンジンが後手のとき）
            m_shogiClock->setPlayer2ConsiderationTime(static_cast<int>(thinkMs));
        } else {
            // 現在手番=後手 → 直前は先手（= エンジンが先手のとき）
            m_shogiClock->setPlayer1ConsiderationTime(static_cast<int>(thinkMs));
        }
...
void MainWindow::startEngineVsHumanGame()
{
    ...
    // この対局で m_usi1 が担当する手番を判定
    const bool engineIsP1 = (m_playMode == EvenEngineVsHuman); // 平手: 先手=エンジン / 駒落ち: 後手=エンジン
    ...
    if (m_playMode == EvenEngineVsHuman) {
        initializeAndStartPlayer1WithEngine1(); // 先手=エンジン
    } else if (m_playMode == HandicapHumanVsEngine) {
        initializeAndStartPlayer2WithEngine1(); // 後手=エンジン
    }

    // 初期 position（ponder 用は必要ならこの後も更新）
    m_positionStr1 = "position " + m_startPosStr + " moves";
    m_positionStrList.append(m_positionStr1);
    m_positionPonder1 = m_positionStr1;
    ...
    const bool engineTurnNow =
        (m_gameController->currentPlayer() == (engineIsP1 ? ShogiGameController::Player1
                                                          : ShogiGameController::Player2));

    // 人間が先に指す局面（駒落ちなど）の場合は、ここでは go を出さず UI を整えて戻る
    if (!engineTurnNow) {
        // 人間手番用のストップウォッチを UI 安定後にアーム
        QTimer::singleShot(0, this, [this]{ armHumanTimerIfNeeded(); });
        updateTurnAndTimekeepingDisplay();
        return;
    }
    ...
    // この手の思考上限（ms）をメモ（旗落ち判定用）
    const bool p1turn_before = (m_gameController->currentPlayer() == ShogiGameController::Player1);
    const int  budgetMsMove  = computeMoveBudgetMsForCurrentTurn();
    ...
    if (isMoveValid) {
        // この手の思考時間と“旗落ち”判定（対象はこの手を指した側のみ）
        const qint64 thinkMs = m_usi1->lastBestmoveElapsedMs();
...
void MainWindow::startEngineVsEngineGame()
{
    // 既存インスタンスの後片付け
    if (m_usi1 != nullptr) { delete m_usi1; m_usi1 = nullptr; }
    if (m_usi2 != nullptr) { delete m_usi2; m_usi2 = nullptr; }
    ...
    QObject::disconnect(m_usi1, &Usi::bestMoveResignReceived, this, &MainWindow::onEngine1Resigns);
    QObject::disconnect(m_usi2, &Usi::bestMoveResignReceived, this, &MainWindow::onEngine2Resigns);
    QObject::connect(m_usi1, &Usi::bestMoveResignReceived, this, &MainWindow::onEngine1Resigns, chooseConn(m_usi1));
    QObject::connect(m_usi2, &Usi::bestMoveResignReceived, this, &MainWindow::onEngine2Resigns, chooseConn(m_usi2));
    ...
    // 初期 position（両エンジンの ponder 用も分離して持つ）
    m_positionStr1 = "position " + m_startPosStr + " moves";
    m_positionStrList.append(m_positionStr1);
    m_positionPonder1 = m_positionStr1;
    ...
    while (!m_gameIsOver) {  // ★ 終局フラグ監視
        // ================= 先手側（Engine1） =================
        if (m_gameIsOver) break; // 念のため

        m_gameController->setPromote(false);
        refreshGoTimes(); // go の btime/wtime を最新化

        // この手の思考上限（ms）を固定でメモ（旗落ち判定用）
        ...
        if (m_gameIsOver) break; // 適用中に終局になった場合の保険

        if (isMoveValid1) {
            const qint64 thinkMs1 = m_usi1->lastBestmoveElapsedMs();

            // 旗落ち判定（この手の手番だった側のみ見る）
        ...
        // ================= 後手側（Engine2） =================
        if (m_gameIsOver) break; // ★ 投了直後に次の go を出さないのが重要
        ...
        m_usi2->handleEngineVsHumanOrEngineMatchCommunication(
            m_positionStr1, m_positionPonder2,   // Engine2 は p2 用の ponder 文字列
            outFrom, outTo,
            m_byoyomiMilliSec1, m_bTime, m_wTime,
            m_addEachMoveMiliSec1, m_addEachMoveMiliSec2, m_useByoyomi
        );
...
void MainWindow::loadKifuFromFile(const QString& filePath)
{
    ...
    m_variationsSeq.clear();   // ★ ツリー用：登場順で保持
    ...
    for (const KifVariation& v0 : res.variations) {
        ...
        // v.sfenList / v.gameMoves 生成（既存ロジック）
        {
...
void MainWindow::rebuildBranchTreeView()
{
    ...
    const qreal TOP_Y   = 40;
    const qreal VAR_Y0  = TOP_Y + 70;
    const qreal ROW_H   = 58;
    ...
    auto addNode = [&](const QRectF& rc, const QString& text,
                       ...
                       int role, int ply = -1, int varIdx = -1, int varOff = -1)
                       -> QGraphicsPathItem*
    {
        QPainterPath path; path.addRoundedRect(rc, RADIUS, RADIUS);
        auto* item = sc->addPath(path, pen, brush);
        item->setZValue(10);                                 // ノードを前面へ
        if (role != NodeHeader) {
        ...
        item->setData(1, ply);       // 本譜ply or 変化での絶対ply
        item->setData(2, varIdx);    // 変化のインデックス（m_variationsSeq内）
        item->setData(3, varOff);    // 変化内オフセット（0起点）
        ...
        t->setParentItem(item);                                     // ← 一緒に選択されるよう親子付け
        return item;
    };
    ...
    for (int i = 1; i <= maxPly; ++i) {
        const QString hdr = QStringLiteral("%1手目").arg(i);
        auto* h = sc->addText(hdr);
        h->setZValue(100);
        const QRectF br = h->boundingRect();
        h->setPos(colX(i) + NODE_W/2.0 - br.width()/2.0, TOP_Y - 28);
    }

    // ---- ペン・ブラシ ----
    QPen   penRoot(QColor(220, 60, 60), 1.0);
    QBrush brRoot(Qt::white);
    QPen   penMain(QColor(70, 120, 170), 1.0);
    QBrush brMain(QColor(230, 246, 255));
    QPen   penVar (QColor(60, 110, 160), 1.0);
    QBrush brVar (QColor(235, 244, 252));

    QHash<int,QRectF> lastRectAtCol;        // その列の直近ノード矩形（親アンカー用）

    // ---- 0列目：開始局面（クリック可） ----
    QRectF rcRoot(colX(0), TOP_Y, NODE_W, NODE_H);
    addNode(rcRoot, QStringLiteral("S"), penRoot, brRoot, NodeRoot);
    lastRectAtCol[0] = rcRoot;

    // ---- 本譜横並び ----
    for (int i = 1; i <= mainN; ++i) {
        QRectF rc(colX(i), TOP_Y, NODE_W, NODE_H);
        addNode(rc, m_dispMain.at(i-1).prettyMove, penMain, brMain, NodeMain, /*ply*/i);

        // 線は矩形の端同士（ノード内部を横切らない）
        addEdge(rightCenter(lastRectAtCol[i-1]), leftCenter(rc));
        lastRectAtCol[i] = rc;
    }

    // 各列の「使用済み段」（本譜=0 段を使用済みに）
    QHash<int,int> usedRows;
    for (int i = 0; i <= maxPly + 2; ++i) usedRows[i] = 1;
    ...
    for (int vi = 0; vi < m_variationsSeq.size(); ++vi) {
        ...
        for (int k = 0; k < v.disp.size(); ++k) {
            const int col = start + k;
            const qreal x = colX(col);
            const qreal y = VAR_Y0 + (rowIdx-1)*ROW_H;
            QRectF rc(x, y, NODE_W, NODE_H);

            addNode(rc, v.disp.at(k).prettyMove, penVar, brVar,
                    NodeVar, /*ply*/start+k, /*varIdx*/vi, /*varOff*/k);

            if (k == 0) {
                // 親（start-1列の直近ノード）から1手目へ
                addEdge(rightCenter(lastRectAtCol.value(start-1, rcRoot)), leftCenter(rc));
            } else {
                // 変化内の水平接続
                addEdge(rightCenter(prevRc), leftCenter(rc));
            }
            prevRc = rc;
            lastRectAtCol[col] = rc; // その列の最新ノードを更新
        }
    }

    // シーン矩形調整
    const QRectF br = sc->itemsBoundingRect().adjusted(-20,-20, 200, 50);
    sc->setSceneRect(br);

    // ---- クリックで遷移（選択変更ハンドラ） ----
    QObject::disconnect(sc, nullptr, this, nullptr);

    QObject::connect(sc, &QGraphicsScene::selectionChanged, this, [this, sc]() {
        const auto items = sc->selectedItems();
        if (items.isEmpty()) return;
        auto* it = items.first();
        const int role = it->data(0).toInt();
        const int ply  = it->data(1).toInt();
        ...
        auto gotoMain = [&](int selPly){
            ...
            *m_sfenRecord = m_sfenMain;
            m_gameMoves   = m_gmMain;
            showRecordAtPly(m_dispMain, /*selectPly=*/selPly);
            m_currentSelectedPly = selPly;
            selectRowInView(selPly);

            // 分岐候補を該当手で更新（0なら空）
            populateBranchListForPly(selPly);

            // 盤・ハイライトを明示同期（古いハイライト残り対策）
            syncBoardAndHighlightsAtRow(selPly);
            enableArrowButtons();
        };
        ...
        if (role == NodeMain) {
            if (ply >= 0) gotoMain(ply);
            return;
        }

        if (role == NodeVar) {
            const int vi = it->data(2).toInt();
            const int off = it->data(3).toInt();
